...
test('精确聚合主要与次要时间窗口的六项总览', () => {
  const summary = aggregateUsage(todayUsage, weekUsage);
  assert.deepEqual(
    {
      mainTokens: summary.main.totalTokens,
      mainCost: summary.main.estimatedCost,
      mainSessions: summary.main.sessionCount,
      mainActiveSeconds: summary.main.activeSeconds,
      secondaryTokens: summary.secondary.totalTokens,
      secondaryCost: summary.secondary.estimatedCost,
    },
    {
      mainTokens: 13_000,
      mainCost: 0.35,
      mainSessions: 2,
      mainActiveSeconds: 2_700,
      secondaryTokens: 51_000,
      secondaryCost: 3.01,
    },
  );
});
...
test('Top 3 工具按 Token 降序且同工具合并', () => {
  const summary = aggregateUsage(todayUsage, weekUsage);
  assert.deepEqual(summary.secondary.topTools, [
...
test('Top 3 模型按 Token 降序且同模型合并', () => {
  const summary = aggregateUsage(todayUsage, weekUsage);
  assert.deepEqual(summary.secondary.topModels, [
...
test('空数据生成明确的暂无数据画面', () => {
  ...
  assert.equal(summary.main.hasAnyData, false);
  assert.equal(summary.secondary.hasAnyData, false);
  assert.equal(payload.data.mainTokens, '暂无数据');
  assert.equal(payload.data.secondaryTokens, '暂无数据');
...
test('主要与次要 Token 共用四位有效数字格式', () => {
  const summary = aggregateUsage(todayUsage, weekUsage);
  summary.main.totalTokens = 12_345;
  summary.secondary.totalTokens = 1_234_567;
  const payload = buildCanvasPayload(summary, new Date('2026-07-28T04:00:00Z'));
  assert.equal(payload.data.mainTokens, '1.235万');
  assert.equal(payload.data.secondaryTokens, '123.5万');
});
